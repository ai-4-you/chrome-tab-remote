...
## 2026-08-26 — macOS native-host productisation research

> Scope: a user-initiated macOS installation with noninteractive setup after explicit user authorization. Managed deployment and automatic self-update are separate decisions.

- **Chrome contract (official):** the native-host manifest needs a valid JSON `name`, `type: "stdio"`, exact non-wildcard `allowed_origins`, and an absolute host `path` on macOS. Chrome supports user-level and system-wide manifest locations; the user-level location is under Chrome’s user profile `NativeMessagingHosts/` directory. The host’s working directory is the host binary directory. Source: <https://developer.chrome.com/docs/extensions/develop/concepts/native-messaging>
- **macOS trust route (official Apple guidance):** sign executable code with Developer ID Application, sign a `.pkg` with Developer ID Installer, use Hardened Runtime and secure timestamps, notarize the final distribution container with `xcrun notarytool`, then staple and validate the `.pkg`. Verify package signatures with `pkgutil --check-signature`. Sources: <https://developer.apple.com/documentation/xcode/packaging-mac-software-for-distribution> · <https://developer.apple.com/documentation/security/notarizing-macos-software-before-distribution>
- **Per-user installation:** Apple’s installer distribution support includes a current-user-home mode; it is a candidate for a user-authorized install without administrator privileges. Its exact payload, script, upgrade, and logged-out behavior must be validated on supported macOS versions before adoption. Do not assume installer scripts have the user’s normal shell environment or a logged-in GUI session.
- **Node SEA is a candidate, not a decision:** Node’s official SEA documentation says the embedded entrypoint defaults to CommonJS; ESM must opt in with `mainFormat: "module"`, and injected code cannot freely load application files from disk. Current Node documentation says SEA CI testing covers macOS arm64 only and skips macOS x64. Therefore it cannot be the sole macOS artifact strategy until both target architectures pass a real proof.
- **Current release conclusion:** prefer a signed/notarized project-owned host bundle installed outside the repository. Select SEA, a bundled Node runtime, or another self-contained artifact only after clean-machine arm64 and x64 proof, protocol compatibility, signing, notarization, size, update, and reproducibility evidence.
