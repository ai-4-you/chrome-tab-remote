...
- **O-9** `tab_find(query, role?)` (implemented 2026-08-02; snapshot-backed behavior live-verified 2026-09-19) searches the latest snapshot's nodes (name/value/URL substring, case-insensitive; optional role) and returns matching snapshot lines (capped 30) — no re-capture or ref invalidation; errors if no snapshot exists yet.
...
- **C-11** **Honest post-action re-orientation (implemented 2026-08-02; receipt behavior live-verified 2026-09-19):** every action result is a dispatch receipt with executed steps, first failure, and page settle state (`settled`, `still-changing`, or `interrupted`), with NO embedded snapshot; observe explicitly via `tab_snapshot` or `tab_find`. No auto-retry of plans — a lost channel could mean actions already ran.
