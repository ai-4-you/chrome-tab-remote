...
- **O-9** `tab_find(query, role?)` (implemented 2026-08-02) searches the latest snapshot's nodes (name/value/URL substring, case-insensitive; optional role) and returns matching snapshot lines (capped 30) — no re-capture or ref invalidation; errors if no snapshot exists yet.
