...
const nativeHostManifest = process.env.CTR_PRODUCTION_NATIVE_HOST_MANIFEST;
const allowedFiles = [
  'background.js',
  'content.js',
  'icon128.png',
...
try {
  ...
  if (!nativeHostManifest) fail('set CTR_PRODUCTION_NATIVE_HOST_MANIFEST to the production native-host manifest.');
  ...
  execFileSync('node', [join(root, 'scripts/verify-store-package.mjs'), '--zip', output, '--public-key', publicKey, '--native-host-manifest', nativeHostManifest], { stdio: 'inherit' });
